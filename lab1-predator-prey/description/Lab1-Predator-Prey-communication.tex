...
Work \textbf{only} in the \code{\bf my} package. If you need anything else, discuss with the assistant.

\task Use your code from the previous activity combined with the new given code (see page \pageref{page2}).

\task Implement in \code{MyEnvironment} the code for the management of messages. Messages sent at one step will be delivered in the next step.
...
\textbf{Helpful pointers:} \begin{itemize}\itemsep1.5ex
	...
	\item For predators, \code{response} should now return an instance of \code{SocialAction} instead of \code{MyAction}. ``Social actions'' contain the physical action to perform (movement) and also messages to be sent to other agents.
	\item Messages from the returned \code{SocialAction} instance must be kept from one step to another in \code{MyEnvironment.messageBox}. \textbf{Careful:} don't store messages from the current step and from the previous step simultaneously in the \code{messageBox}. 
\end{itemize}
