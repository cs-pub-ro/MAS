...
logger = logging.getLogger("blocksworld")
...
class BlockStack(object):
    ...
    def sane(self) -> bool:
        if not self.blocks and not self.locked_blocks:
            logger.error("This stack contains no blocks")
            return False
        return True
    ...
    def is_on(self, top_block: Block, bottom_block: Block) -> bool:
        if not self.sane():
            return False

        if top_block not in self.get_blocks():
            logger.error("This stack does not contain block %s" % str(top_block))
            return False

        if bottom_block not in self.get_blocks():
            logger.error("This stack does not contain block %s" % str(bottom_block))
            return False
    ...
    def get_above(self, block: Block) -> Block:
        if block not in self.get_blocks():
            logger.error("Block [%s] is not in this stack" % str(block))
            return None
    ...
    def unstack(self, to_unstack: Block, unstack_from: Block) -> Tuple[Block or None, int]:
        """
        ...
        """
        if not self.sane():
            return None, REWARD_INVALID

        if not self.get_blocks():
            logger.error("All blocks in this stack are locked.")
            return None, REWARD_INVALID

        if self.get_blocks() and self.get_blocks()[-1] == to_unstack:
            if self.is_single_block():
                logger.error("Block [%s] is directly on the table. Use pickup." % str(to_unstack))
                return None, REWARD_INVALID

            if not self.get_blocks()[-2] == unstack_from:
                logger.error("Block [%s] is not over [%s]." % (str(to_unstack), str(unstack_from)))
                return None, REWARD_INVALID

            return self.blocks.pop(), REWARD_VALID

        logger.error("Block [%s] is not the topmost block of this stack" % str(to_unstack))
        return None, REWARD_INVALID

    def stack(self, to_stack: Block, stack_over: Block) -> int:
        if not self.sane():
        ...
        if self.get_blocks()[-1] == stack_over:
            ...
            logger.error("Block [%s] is not the topmost block of this stack" % str(stack_over))
            return REWARD_INVALID

    def lock(self, block: Block) -> int:
        """
        ...
        if block in self.locked_blocks:
            logger.error("Block [%s] is already locked" % str(block))
            return REWARD_INVALID

        if not self.is_on_table(block) and not self.get_below(block) in self.locked_blocks:
            logger.error("The block under [%s] is not locked" % str(block))
            return REWARD_INVALID
...
class BlocksWorld(object):
    ...
    def get_stack(self, block: Block) -> Tuple[BlockStack or None, int]:
        """
        ...
        if not block in self.all_blocks:
            logger.error("Block [%s] has never existed in this world\n %s" % (str(block), str(self)))
            return None, REWARD_INVALID

        for stack in self.stacks:
            if block in stack.get_blocks():
                return stack, REWARD_VALID

        logger.error("Block [%s] is not currently in any stack" % str(block))
        return None, REWARD_INVALID
    ...
    def pick_up(self, block: Block) -> Tuple[Block or None, int]:
        stack, _ = self.get_stack(block)

        if not stack:
            return None, REWARD_INVALID

        if not stack.is_single_block():
            logger.error("Block [%s] is not in a single-block stack. Use unstack instead " % str(block))
            return None, REWARD_INVALID

        self.stacks.remove(stack)
        return stack.get_top_block(), REWARD_VALID

    def put_down(self, block: Block, current_stack: BlockStack) -> Tuple[BlockStack or None, int]:
        if block not in self.all_blocks:
            logger.error("Block [%s] has never existed in this world" % str(block))
            return None, REWARD_INVALID
    ...
    def unstack(self, to_unstack: Block, unstack_from: Block) -> Tuple[Block or None, int]:
        stack, _ = self.get_stack(to_unstack)
        if stack:
            return stack.unstack(to_unstack=to_unstack, unstack_from=unstack_from)

        return None, REWARD_INVALID

    def stack(self, to_stack: Block, stack_over: Block) -> int:
        stack, _ = self.get_stack(stack_over)
        if stack:
            return stack.stack(to_stack=to_stack, stack_over=stack_over)
    ...
    def lock(self, block: Block) -> int:
        stack, _ = self.get_stack(block)
        if stack:
            return stack.lock(block)
        return REWARD_INVALID

    def is_on_table(self, block: Block) -> bool:
        stack, _ = self.get_stack(block)
        if stack:
            return stack.is_on_table(block)
        return False
    ...
    def clone(self) -> 'BlocksWorld':
        ...
        world = BlocksWorld()
        world.all_blocks = set(deepcopy(self.get_all_blocks()))
        world.stacks = []
    ...
    def get_state(self):
        ...
        stacks_state = []
        for stack in self.stacks:
            stacks_state.append(tuple(stack.get_blocks()))
